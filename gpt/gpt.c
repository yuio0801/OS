...
int global_B, global_T, global_C, global_OC; 
float* global_out, * global_inp, * global_weight, * global_bias;
int block_size, consumer_sum;
struct Point *st; int tail;

void T_produce(int tid)
{
    for (int b = 0; b < global_B; b++) {
            for (int t = 0; t < global_T; t++) {            
                mutex_lock(&lk);
                st[++tail] = (struct Point){b, t};
                cond_broadcast(&cv);
                mutex_unlock(&lk);
            }
        }
...
void T_consume(int tid) {
    while (1) {
        mutex_lock(&lk);
        while (consumer_sum != global_B * global_T && !tail) {
            cond_wait(&cv, &lk);
        }
        if(consumer_sum == global_B * global_T)
        {
            assert(!tail);
            cond_broadcast(&cv);
            mutex_unlock(&lk);
        ...
        assert(tail);
        struct Point tmp = st[tail--];
        consumer_sum++;
        cond_broadcast(&cv);
        mutex_unlock(&lk);

        int b = tmp.b, t = tmp.t; 
        float* out = global_out, *inp = global_inp, * weight = global_weight, * bias = global_bias;
        int B = global_B, T = global_T, C = global_C, OC = global_OC;
...
void matmul_forward(float* out,
    ...
    st = (struct Point *)malloc((B * T + 1 )* sizeof(struct Point));

    tail = 0, consumer_sum = 0;

    int producer_num = 1;
    int consumer_num = 4;
    global_B = B; global_T = T; global_C = C; global_OC = OC;
    global_out = out; global_inp = inp; global_weight = weight; global_bias = bias;
    ...
    for(int i = 0; i < 1; ++i)
    {
        ...
        pthread_create(
            &(pp.thread),  // a pthread_t
            NULL,  // options; all to default
            wrapper_,  // the wrapper function
            &pp// the argument to the wrapper
        );
    }
    struct thread threads_[6];
    for(int i = 0; i < 6; ++i)
    {
        threads_[i] = (struct thread) {
            .id = i + 1,
            .status = T_LIVE,
            .entry = T_consume,
        };
        pthread_create(
            &(threads_[i].thread),  // a pthread_t
            NULL,  // options; all to default
            wrapper_,  // the wrapper function
            &threads_[i] // the argument to the wrapper
        );
    }
    struct thread *t = &pp;
    if (t->status == T_LIVE) {
